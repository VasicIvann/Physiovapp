...
export const computeDailyPoints = (log: DailyLogEntry) => {
  ...
  // Nutrition (score moyen des 3 axes, de 1 à 10)
  const nc = log.nutritionCalorieScore;
  const np = log.nutritionProteinScore;
  const nq = log.nutritionQualityScore;
  if (typeof nc === "number" && typeof np === "number" && typeof nq === "number") {
    const avg = (nc + np + nq) / 3;
    if (avg > 7) points += 2;
    else if (avg > 5) points += 1;
    else if (avg < 4) points -= 2;
    else if (avg < 5) points -= 1;
  }
